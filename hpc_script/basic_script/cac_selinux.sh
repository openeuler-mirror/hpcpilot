...
#!/usr/bin/env bash
######################################################################
# 脚本描述：selinux的主要作用就是最大限度地减小系统中服务进程可访问的资源关闭selinux#
#         安全子系统内核模块自动化脚本                                     #
# 注意事项：无                                                          #
######################################################################
# set -x
...
function close_selinux() {
    ...
    if [ $(is_file_exist ${config_file_path}) != "0" ]; then
        log_warn "[${config_file_path}] file doesn't exist, creating and config..." false
        # 创建文件并赋予权限
    ...
    if [ $(get_property_value "${config_file_path}" ${cons_selinux_key}) != ${cons_selinux_value} ]; then
        ...
        log_warn "[${cons_selinux_key}] value has been configured." false
    fi
    ...
    if [ $(get_property_value "${config_file_path}" ${cons_SELINUXTYPE_key}) != ${cons_SELINUXTYPE_value} ]; then
        ...
        log_warn "[${cons_SELINUXTYPE_key}] value has been configured." false
    fi
}
...
manual_script_action ${is_manual_script} ${is_open_debug} required_check check_selinux_result close_selinux_result ${4}
